...
type Settler interface {
	Settle(ctx context.Context, request *types.SettlementRequest) (*types.SettlementResult, error)
}
...
func (s *SettlementService) AddCosmosClient(network types.Network, client *clients.MinimalCosmosClient) error {
	if !network.IsCosmos() {
		return &types.X402Error{
			Code:    types.ErrUnsupportedNetwork,
...
func (s *SettlementService) Settle(
	...
	switch {
	case network.IsEVM():
		return s.settleEVMPayment(settleCtx, request)
	case network.IsSolana():
		return s.settleSolanaPayment(settleCtx, request)
	case network.IsCosmos():
		return s.settleCosmosPayment(settleCtx, request)
	default:
		return &types.SettlementResult{
			Success: false,
...
func (s *SettlementService) BatchSettle(
	...
	for i, request := range requests {
		go func(index int, req *types.SettlementRequest) {
...
func (s *SettlementService) validateSettlementRequest(request *types.SettlementRequest) error {
	if request == nil {
